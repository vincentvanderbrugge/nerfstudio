...
@dataclass
class NerfactoL1ModelConfig(ModelConfig):
    """Nerfacto Model Config"""

    _target: Type = field(default_factory=lambda: NerfactoL1Model)
...
class NerfactoL1Model(Model):
    ...
    def populate_modules(self):
        ...
        self.renderer_depth_training = DepthRenderer(method='expected')
        self.renderer_depth_evaluation = DepthRenderer()
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        depth_expected = self.renderer_depth_training(weights=weights, ray_samples=ray_samples)
        depth = self.renderer_depth_evaluation(weights=weights, ray_samples=ray_samples)
        ...
        for i in range(self.config.num_proposal_iterations):
            outputs[f"prop_depth_{i}"] = self.renderer_depth_evaluation(weights=weights_list[i], ray_samples=ray_samples_list[i])
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        depth = batch["depth"].to(self.device)
        not_nan_filter = ~torch.isnan(depth)
        depth_loss = self.depth_loss(outputs["depth_expected"][:, 0][not_nan_filter],
                                          depth[not_nan_filter])
